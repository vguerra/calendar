...
ad_proc -public calendar::before-uninstantiate { 
	-package_id:required 
} {
	# get calendar id's
	set calendar_list [db_list _ "SELECT calendar_id FROM calendars WHERE package_id=:package_id"]
	# delete calendars
	foreach calendar_id $calendar_list {
		# delete all calendar items
		db_foreach _ "SELECT cal_item_id FROM cal_items WHERE on_which_calendar=:calendar_id" {
			calendar::item::delete -cal_item_id $cal_item_id
		}
		# delete item types
		db_foreach _ "SELECT item_type_id FROM cal_item_types WHERE calendar_id=:calendar_id" {
...
ad_proc -public calendar::attachments_enabled_p {
	...
	#Learn@WU Michael - nicht funktionierendes Feature nicht anzeigen
	return 0
    if { [empty_string_p $calendar_id] } {
		set package_id [site_node_apm_integration::child_package_exists_p -package_key attachments]
    } else {
    	set calendar_package_id [db_string _ "SELECT package_id FROM calendars WHERE calendar_id=:calendar_id"]
        set package_id [site_node_apm_integration::child_package_exists_p -package_id $calendar_package_id -package_key attachments]
    }
}
...
#     set calendar_id $cal_item(calendar_id)
#     set time_p $cal_item(time_p)
#     set community_obj [::tlf_global load_community [search::dotlrn::get_community_id -package_id $cal_item(calendar_package_id)]]
#     set community_pretty_name ""
#     if {$community_obj ne 0} {
#         set community_pretty_name "\[[$community_obj pretty_name]\] "
#     }
...
# 	    # learn@WU fix für synchronisierte Termine
# 	    if {$cal_item(creation_user) > 0} {
# 		    acs_user::get -user_id $cal_item(creation_user) -array user_info
# 		    set user_info_html "<b>#calendar.created_by#:</b> <a href=\"mailto:$user_info(email)\">$user_info(first_names) $user_info(last_name)</a><br>\n"
# 		  } else {
# 		  	set user_info_html "<b>#calendar.created_by#:</b> VVZ <br>\n"
# 		  }
# 	} else {
# 		set sql_str {
# 			select node_id from site_nodes s, calendars c 
# 			where s.object_id = c.package_id and c.calendar_id = :calendar_id
# 		}
# 		set node_id [db_string get_data $sql_str]
# 		array set cal_data [site_node::get -node_id $node_id]
# 		set url "[ad_url]$cal_data(url)"
# 		set instance_name $cal_data(instance_name)
# 	    set object_id $cal_data(object_id)
# 	    set user_info_html "<b>#calendar.created_by#:</b> Sync-Event<br>\n"
# 	}

#     set new_content ""
#     # append new_content "[_ calendar.Calendar]:  <a href=\"${url}\">$instance_name</a><br>\n"
#     # append new_content "[_ calendar.Calendar_Item]: <a href=\"${url}cal-item-view?cal_item_id=$cal_item_id\">$cal_item(name)</a><br>\n"
#     # append new_content "[_ calendar.Start_Time]: $cal_item(start_date_ansi) $cal_item(start_time)<br>\n"
#     # append new_content "[_ calendar.to]: $cal_item(end_date_ansi) $cal_item(end_time)<br>\n"
...
#     # append new_content $cal_item(description)
#     # append new_content "\n<br>\n"
#     # append new_content $user_info_html

#     append new_content "<b>#calendar.notification_title#:</b> <a href='${url}cal-item-view?cal_item_id=$cal_item_id'>$cal_item(name)</a><br>"
#     append new_content "<b>#calendar.start#:</b> $cal_item(start_date_ansi)<br>\n"
#     append new_content "<b>#calendar.end#:</b> $cal_item(end_date_ansi)<br>\n"
#     if {![empty_string_p $repeat_p] && $repeat_p} {
#         append new_content "#calendar.is_recurring#<br>"
#     }
#     append new_content "<b>#calendar.Description#:</b> $cal_item(description)<br>\n"
#     append new_content $user_info_html
    
#     # send text for now.
#     # set new_content [ad_html_to_text -- $new_content]

# 	# Learn@WU(peter): we have to get the data for the calendar, because we have to know if this is a private calendar
# 	calendar::get -calendar_id $calendar_id -array cal_info

# 	# Learn@WU(peter): if this is a private calender, only notify the user who is the owner of the calendar
# 	if { $cal_info(private_p) == "t" } {
# 	    # Learn@WU(peter): Do the notification for the calendar item and one specific user
# 	    notification::new \
# 	        -type_id [notification::type::get_type_id \
# 	        -short_name calendar_notif] \
# 	        -object_id $object_id \
# 	        -response_id $cal_item(cal_item_id) \
# 	        -notif_subject "$community_pretty_name #calendar.$mode# [_ calendar.Calendar_Item]: $cal_item(name)" \
# 	        -notif_html $new_content \
# 	        -action_id $calendar_id \
# 	        -subset [list $cal_info(owner_id)]
# 	} else {
# 	    # Do the notification for the calendar item
# 	    notification::new \
# 	        -type_id [notification::type::get_type_id \
# 	        -short_name calendar_notif] \
# 	        -object_id $object_id \
...
ad_proc -public calendar::get_ancestor_package_ids {
	package_ids
} {
	Returns list of IDs of calendar packages that exist in ancestors in the site_map
} {
	set result $package_ids
	
	foreach package_id $package_ids {
		set community_id [dotlrn_community::get_community_id]
		if { [empty_string_p $community_id] } {
			set community_id [dotlrn_community::get_community_id -package_id [site_node::closest_ancestor_package -node_id [site_node::get_node_id_from_object_id -object_id $package_id] -package_key dotlrn]]
			if { [empty_string_p $community_id] } { continue }
		}
		while {$package_id} {
			set parent_community_id [dotlrn_community::get_parent_id -community_id $community_id]
			if { [empty_string_p $parent_community_id] || $parent_community_id == $community_id} { break }
			set parent_package_id [dotlrn_community::get_applet_package_id -community_id $parent_community_id -applet_key dotlrn_calendar]
			if { [empty_string_p $parent_package_id] || $parent_package_id == $package_id} { break }
			if { [lsearch -exact $result $parent_package_id] == -1 } { lappend result $parent_package_id }
			set community_id $parent_community_id
			set package_id $parent_package_id
		}
	}
	return $result
}
