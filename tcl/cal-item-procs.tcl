...
ad_proc -public calendar::item::new {
    ...
    if {[dates_valid_p -start_date $start_date -end_date $end_date]} {
        ...
        # If a unique id is given, insert it into the mapping table
        if {![empty_string_p $cal_uid]} {
...
ad_proc -public calendar::item::edit {
    ...
    if {[dates_valid_p -start_date $start_date -end_date $end_date]} {
        ...
        db_transaction {
           # If a unique id is given, update vars in the mapping table
           if {![empty_string_p $cal_uid]} {
             db_dml update_ical_vars_for_cal_uid {} 
           }
...
ad_proc -public calendar::item::delete {
    ...
    set activity_id [db_string select_activity {}]
    db_exec_plsql delete_cal_item {}

    # When the last cal_item is deleted, delete the activity as well;
    # This is symetrically to ::calendar::item::new, where an activity
    # is created.
ns_log notice "+++ count items = [db_string count_items {}], activity_id=$activity_id"
    # should there be a delete cascade on the fk from events to activities???
    set f dbqd.calendar.tcl.cal-item-procs.calendar::item::delete
    if {[db_string $f.count_items {}] == 0} {
      db_dml $f.delete_activity {}
    }
}
